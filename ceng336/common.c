...
void transmitBuffer_push(char *str,unsigned char size, char isMotion) // pushes reverse str to buffer. UNSAFE! LOCK FIRST!
{
    ...
    excuse_me = 0;
    if (isMotion)
        SetEvent(TRANSMIT_ID, GO_TRANSMIT);
    else
...
void transmitData()
{
    ...
    {
        SetEvent(MOVEBOT_ID, TRANSMIT_END);
...
void dataReceived()
{
    ...
    else if(receive_state == _OPERATING && rxbyte == ':')
    {
       ...
       SetEvent(MOVEBOT_ID, ALARM_EVENT);
    }
